 ...
 * @param addurl//目标页面地址
 */
function openConfirmWindow(title,addurl,gname,width,height,callback) {
	gridname=gname;
	createwindow(title, addurl,width,height,callback);
}
/**
 * 树列表添加事件打开窗口
 * @param title 编辑框标题
 ...
 * @param saveurl
 */
function createwindow(title, addurl,width,height,callback) {
	width = width?width:700;
	height = height?height:400;
	if(width=="100%" || height=="100%"){
		width = window.top.document.body.offsetWidth;
	...
	if(typeof(windowapi) == 'undefined'){
		$.dialog({
			...
			cache:false,
			button: [
			   {name: '确定', callback: function(){eval(callback)}, focus: true},
			   {name: '取消', focus: true}
			],
			//ok: function(){
		...
		W.$.dialog({
			...
			cache:false,
			button: [
			   {name: '确定', callback: function (){alert(1)}, focus: true},
			   {name: '取消', callback: function (){alert(1)}, focus: true}
			]
		    //ok: function(){
		    //	iframe = this.iframe.contentWindow;
			//	saveObj();
