...
jQuery.extend({
			...
			ajaxFileUpload : function(s) {
				...
				fileObj.fileName=fielName.substring(fielName.lastIndexOf("\\")+1,fielName.lastIndexOf("."));
				var extention = fielName.substring(fielName.lastIndexOf(".") + 1).toLowerCase();
				fileObj.fileType=extention;//获取文件后缀
				if (s.fileFilter && s.fileFilter.indexOf(extention) < 0) {
					$.messager.alert("消息","仅支持 (" + s.fileFilter + ") 为后缀名的文件!" );
					process.off();
					return;
				}
				//文件大小限制
				if (s.fileSize > 0) {
					...
					//fileSize0 = fs/(1024*1024);
					//fileSize0 = fileSize0.toFixed(2);
					fileSize1 = s.fileSize/1024;
					console.log('上传文件大小为：',fs);
					if (fs > s.fileSize*1024) {
						$.messager.alert("警告","当前文件大小超过允许的限制值 (" + fileSize1+ "M)！");
						process.off();
						return;
					}
				}
				...
				var uploadCallback = function(isTimeout) {
					...
					try {
						...
						if (io.contentWindow) { //兼容各个浏览器，可取得子窗口的 window 对象
							xml.responseText = io.contentWindow.document.body ? io.contentWindow.document.body.innerText // mao innerHTML > innerText 解决返回数据带pre标签问题
									: null;
